...
class A16 : public BaseProject {
  ...
  MeshUniformBlock uboRoom;
    ...
    DSVColor.bind(commandBuffer, PVColor, 1, currentImage);
		vkCmdDrawIndexed(commandBuffer,
...
//std::cout << Wheel1Rot << " --- " << TargetRot << "\n";
		...
		const float FOVy = glm::radians(90.0f / 2.0f);
		...
		World = glm::mat4(1);
		uboRoom.amb = 1.0f; uboBody.gamma = 180.0f; uboBody.sColor = glm::vec3(1.0f);
		uboRoom.mvpMat = Prj * View * World;
		uboRoom.mMat = World;
		uboRoom.nMat = glm::inverse(glm::transpose(World));
		/* map the uniform data block to the GPU */
		DSVColor.map(currentImage, &uboRoom, sizeof(uboRoom), 0);
