...
// Questi dovrebbero essere definibili a piacimento e nel modo piu' semplice
// possibile, traferirsi nello shader
...
private:
  ...
  static void framebufferResizeCallback(GLFWwindow *window, int width,
                                        int height) {
    auto app =
        reinterpret_cast<Assignment01 *>(glfwGetWindowUserPointer(window));
  ...
  void createDescriptorSetLayouts() { createPhongDescriptorSetLayout(); }
  ...
  void createPipelines() { createPhongPipeline(); }

  void createPhongPipeline() {
    createPipeline("PhongVert.spv", "PhongFrag.spv", PhongPipelineLayout,
                   PhongPipeline, PhongDescriptorSetLayout,
  ...
  void createTextureImage(const char *FName, TextureData &TD) {
    ...
    stbi_uc *pixels = stbi_load((TEXTURE_PATH + FName).c_str(), &texWidth,
                                &texHeight, &texChannels, STBI_rgb_alpha);
    if (!pixels) {
      std::cout << (TEXTURE_PATH + FName).c_str() << "\n";
      throw std::runtime_error("failed to load texture image!");
    }
    std::cout << FName << " -> size: " << texWidth << "x" << texHeight
              << ", ch: " << texChannels << "\n";

    VkDeviceSize imageSize = texWidth * texHeight * 4;
    TD.mipLevels = static_cast<uint32_t>(
    ...
    createImage(texWidth, texHeight, TD.mipLevels, VK_SAMPLE_COUNT_1_BIT,
    ...
    transitionImageLayout(
        TD.textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED,
        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, TD.mipLevels, 1);
    copyBufferToImage(stagingBuffer, TD.textureImage,
                      static_cast<uint32_t>(texWidth),
                      static_cast<uint32_t>(texHeight), 1);

    generateMipmaps(TD.textureImage, VK_FORMAT_R8G8B8A8_SRGB, texWidth,
                    texHeight, TD.mipLevels, 1);
  ...
  void createTextureImageView(TextureData &TD) {
    TD.textureImageView = createImageView(
        TD.textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT,
        TD.mipLevels, VK_IMAGE_VIEW_TYPE_2D, 1);
  }
  ...
  void createTextureSampler(TextureData &TD) {
    ...
    VkResult result =
        vkCreateSampler(device, &samplerInfo, nullptr, &TD.textureSampler);
    if (result != VK_SUCCESS) {
      PrintVkError(result);
    ...
    for (const auto &shape : shapes) {
      for (const auto &index : shape.mesh.indices) {
        ...
        if (uniqueVertices.count(vertex) == 0) {
          uniqueVertices[vertex] = static_cast<uint32_t>(MD.vertices.size());
  ...
  void createVertexBuffer(ModelData &Md) {
    ...
    createBuffer(bufferSize,
                 ...
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, Md.vertexBuffer,
                 Md.vertexBufferMemory);
  ...
  void createIndexBuffer(ModelData &Md) {
    ...
    createBuffer(bufferSize,
                 ...
                 VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, Md.indexBuffer,
                 Md.indexBufferMemory);
  ...
  void createDescriptorPool() {
    ...
    poolSizes[0].descriptorCount =
        static_cast<uint32_t>(swapChainImages.size() * Scene.size());
    ...
    poolInfo.maxSets =
        static_cast<uint32_t>(swapChainImages.size() * (Scene.size() + 1));
  ...
  void createDescriptorSets() { createPhongDescriptorSets(); }

  void createPhongDescriptorSets() {
    std::vector<VkDescriptorSetLayout> layouts(
        swapChainImages.size() * Scene.size(), PhongDescriptorSetLayout);
    ...
    for (size_t i = 0; i < commandBuffers.size(); i++) {
      ...
      for (int j = 0; j < Scene.size(); j++) {
        ...
        vkCmdBindDescriptorSets(
            ...
            &PhongDescriptorSets[i + j * swapChainImages.size()], 0, nullptr);

        vkCmdDrawIndexed(commandBuffers[i],
                         static_cast<uint32_t>(Scene[j].MD.indices.size()), 1,
                         0, 0, 0);
      }
  ...
  void updateUniformBuffer(uint32_t currentImage) {
    ...
    // std::cout << "Cam Pos: " << CamPos[0] << " " << CamPos[1] << " " <<
    // CamPos[2] << "\n";

    glm::mat4 CamMat =
        glm::translate(glm::transpose(glm::mat4(CamDir)), -CamPos);
    ...
    for (int j = 0; j < Scene.size(); j++) {
      ...
      //
      // for(int ti=0;ti<3;ti++){for(int tj=0;tj<3;tj++){std::cout <<
      // ubo.nMat[ti][tj] << " ";}std::cout<<"\n";}std::cout<<"\n"; glm::mat4 tM
      // = glm::inverse(glm::transpose(ubo.mMat)); for(int
      // ti=0;ti<4;ti++){for(int tj=0;tj<4;tj++){std::cout << tM[ti][tj] << "
      // ";}std::cout<<"\n";}std::cout<<"\n";
    ...
    gubo.lightColor =
        (Mode == 0
             ...
             : (Mode == 1 ? glm::vec4(0.0f, 1.0f, 0.0f, 1.0f)
                          : (Mode == 2 ? glm::vec4(0.0f, 0.0f, 1.0f, 1.0f)
