...
void main() {
  ...
  vec3 unNormLightDir = gubo.lightPos - fragPos;
	vec3 lightDir = normalize(unNormLightDir);

  float num = dot(lightDir, gubo.lightDir) - cosout;
  float denom = cosin - cosout;
  float coneDimming = clamp(num / denom, 0.0, 1.0);
  vec3 lightDimming = pow(g / length(unNormLightDir), beta) * gubo.lightColor.rgb;
	vec3 lightColor = lightDimming * coneDimming;
