...
protected:
  ...
  static void framebufferResizeCallback(GLFWwindow *window, int width,
                                        int height) {
    auto app =
        reinterpret_cast<BaseProject *>(glfwGetWindowUserPointer(window));
  ...
  public:
    ...
    void print() {
      ...
      std::cout << "swapChainPresentModeSupport: "
                << swapChainPresentModeSupport << "\n";
  ...
  // Debug commands
  void printVec3(const char *Name, vec3 v) {
    std::cout << "vec3 " << Name << " = glm::vec3(" << v[0] << ", " << v[1]
              << ", " << v[2] << ");\n";
  }
  void printVec4(const char *Name, vec4 v) {
    std::cout << "vec4 " << Name << " = glm::vec4(" << v[0] << ", " << v[1]
              << ", " << v[2] << ", " << v[3] << ");\n";
  }
  void printMat3(const char *Name, mat3 v) {
    std::cout << "mat3 " << Name << " = glm::mat3(";
    for (int i = 0; i < 9; i++) {
      std::cout << v[i / 3][i % 3] << ((i < 8) ? ", " : ");\n");
    }
  }
  void printMat4(const char *Name, mat4 v) {
    std::cout << "mat4 " << Name << " = glm::mat4(";
    for (int i = 0; i < 16; i++) {
...
void Model::bind(VkCommandBuffer commandBuffer) {
  ...
  // property .indexBuffer of models, contains the VkBuffer handle to its index
  // buffer
...
void Texture::createTextureImage(const char *const files[],
  ...
  for (int i = 0; i < imgs; i++) {
    ...
    std::cout << "[" << i << "]" << files[i] << " -> size: " << texWidth << "x"
              << texHeight << ", ch: " << texChannels << "\n";
  ...
  BP->createImage(
      texWidth, texHeight, mipLevels, imgs, VK_SAMPLE_COUNT_1_BIT, Fmt,
      VK_IMAGE_TILING_OPTIMAL,
...
void Texture::createTextureImageView() {
  textureImageView = BP->createImageView(
      textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT,
      mipLevels, imgs == 6 ? VK_IMAGE_VIEW_TYPE_CUBE : VK_IMAGE_VIEW_TYPE_2D,
      imgs);
}
...
void DescriptorSet::map(int currentImage, void *src, int size, int slot) {
  ...
  vkMapMemory(BP->device, uniformBuffersMemory[slot][currentImage], 0, size, 0,
              &data);
